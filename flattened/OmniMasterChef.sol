...
// File: contracts\OmniToken.sol

pragma solidity 0.6.12;


// OmniToken with Governance.
contract OmniToken is BEP20('OmniSwap', 'OMNISWAP') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "OMNISWAP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "OMNISWAP::delegateBySig: invalid nonce");
        require(now <= expiry, "OMNISWAP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "OMNISWAP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying OMNISWAPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "OMNISWAP::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts\OmniSyrupBar.sol

pragma solidity 0.6.12;



// OmniSyrupBar with Governance.
contract OmniSyrupBar is BEP20('OmniSwap SyrupBar Token', 'OMNISYRUP') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safeOmniTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 omniBal = omniSwap.balanceOf(address(this));
        if (_amount > omniBal) {
            omniSwap.transfer(_to, omniBal);
        } else {
            omniSwap.transfer(_to, _amount);
        }
    }
...
// File: contracts\OmniMasterChef.sol
